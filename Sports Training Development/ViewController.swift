...
class ViewController: UIViewController {
    ...
    @IBAction func onOffButton(_ sender: UIButton) {
        ...
        if isOn {
            ...
            self.backgroundColorLabel.isHidden = true
            isFirstTime = true
        ...
        if(!isFirstTime){
            if(!testingOn){
                ...
                if ((angleDiff <= 45.0) && (angleDiff >= -45.0)){
                    backgroundChange(Score: true)
                    points += 10
                    pointsLabel.text = "Points: \(points)"
                }else{
                    backgroundChange(Score: false)
                }
                timesGone += 1
    ...
    @objc func backgroundSounds() {
        bg.play()
    }
    
    func backgroundChange(Score: Bool) {
        backgroundColorLabel.layer.zPosition = -1
        backgroundColorLabel.isUserInteractionEnabled = false //  Moving changing background to back
        
        if (Score == true) {
            //  green
            self.backgroundColorLabel.backgroundColor = UIColor(red: 30/255, green: 130/255, blue: 76/255, alpha: 1)
        }else{
            //  orange
            self.backgroundColorLabel.backgroundColor = UIColor(red: 242/255, green: 121/255, blue: 53/255, alpha: 1)
        }
        
        if (!isFirstTime){
            self.backgroundColorLabel.isHidden = false
        }
